...
class StrikingPositionAgent(MinMaxAgent):
    def evaluate_game(self, board, max_player):
        ...
        white_striking_sum = len([1 for pos in list(chain(board.get_possible_moves().values()))[0] if pos and pos[0] == 0])

        new_board.current_player = Player.BLACK
        black_striking_sum = len([1 for pos in list(chain(board.get_possible_moves().values()))[0] if pos and pos[0] == 7])
